...
FITOM_X用のオフラインパッチ/プロファイルエディタ(今後実装予定)のための、
JSONデータモデル/I/O層です。FITOM_X本体(音源エンジン)の一部ではなく、
FITOM_Xが使うのと同じ `*.profile.json` / `*.patchbank.json` /
`*.hwbank.json` / `*.swbank.json` / `*.drumkit.json` /
`*.samplezonebank.json` を読み書きするスタンドアロンのC++ライブラリです。
これにより、FITOM_Xを起動しなくてもパッチの閲覧・編集・保存ができる
エディタを、この上に構築できます。

## 今回の納品範囲

今回納品するのは **データモデルとJSON読み込み/保存層**、および
プロファイルと全バンクをひとまとめの閲覧・編集可能なオブジェクトグラフに
束ねる `PatchWorkspace` クラスのみです。以下は意図的に含んでいません。

- GUI(Dear ImGuiなど)は一切なし。
- 起動中のFITOM_X本体との名前付きパイプ/ソケット接続によるリアルタイム
  試聴機能(FITOM_X側プロジェクトの `docs/plugin-midi-pipe.md` に仕様あり。
  ワイヤーフォーマットは単純な生MIDIバイト列で、Windowsは
  `\\.\pipe\FITOM_X_MIDI`、Linux/macOSは `/tmp/fitom_x_midi.sock` に
  送信するだけなので、後から追加しやすいはずです)。

どちらも、今回のドロップをレビューしやすい範囲に留めるため意図的に
見送りました。データモデルは、`PatchWorkspace` の上にGUI層を変更なしで
載せられるよう設計してあります。
...
tests/smoke_test.cpp 読み込み/CRUD/保存/再読み込みのラウンドトリップ確認テスト(下記「テスト」参照)
fixtures/            スモークテスト用の手書きサンプルプロファイル+バンク
```

## 閲覧・編集階層

`PatchWorkspace` は、エディタUI向けに依頼いただいた階層をそのまま公開します。
...
(元の仕様にある「デバイスパッチバンク → デバイスボイスパッチ →
パフォーマンスバンク → パフォーマンスパッチ」は、別ストレージではなく
**参照**です。`HwPatch` の `sw_bank`/`sw_prog` が、同じ
`performanceBanks()` リスト内を指し示します。`PatchWorkspace::
resolvePerformancePatch()` がUI向けにこの参照をたどります。)
...
今回のセッションで確認済みの内容: `-Wall -Wextra` でクリーンビルド
(警告ゼロ)、スモークテストは85/85チェック通過(読み込み・フィールド
デフォルト・全階層でのCRUD・save-as+再読み込みのラウンドトリップ)。
注意: ネットワークマウントされたドライブ上で直接ビルドすると、環境に
よってはCMakeの依存関係ダウンロード時にファイルシステムの権限エラーが
出ることがあります。その場合はローカルディスク上でビルドしてください。
...
本ライブラリはこれらのドキュメントのみから構築しており、今回のセッションでは
FITOM_X本体のソースツリー自体(および実際の `profile.schema.json` /
`hwbank.schema.json` / `swbank.schema.json` / `patchbank.schema.json` /
`config_schema/drumkit.schema.json`)にはアクセスできませんでした。
本番で使う前に、以下の2点は実際のスキーマと突き合わせて確認することを
おすすめします。
