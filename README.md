...
`fitom_patch_editor_gui` は第1引数に `*.profile.json` のパスを渡すと、
起動時にそのプロファイルを読み込み、メニュー/ファイルブラウザを経由
せず直接アウトライン画面から開始します(ファイルブラウザで選択した
場合と同じ状態)。読み込みに失敗した場合は通常のエラーポップアップを
表示し、メニュー画面にフォールバックします。

```
fitom_patch_editor_gui.exe "C:\path\to\some.profile.json"
```

動作中のFITOM_X本体から、自身が現在読み込んでいるプロファイルの
パスを渡してこのエディタを子プロセスとして起動する、という利用を
想定しています(引数無しの場合は従来通りメニューから始まります)。
