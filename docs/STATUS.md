...
- **パッチ編集フォーム・CRUD UI** — メニュー→ファイルブラウザ→
  ...
  ドラムノートの編集フォーム、バンク/パッチの
  複製・削除UI、プロファイル自体の新規作成・削除UI(メニューの
  「新規プロファイル作成」「プロファイル削除」ボタンは表示のみで
  無効化してある)は未着手。`fpe::PatchWorkspace`のCRUD API
  (複製・削除含む)は揃っているので、これを呼ぶUIを`apps/gui/main.cpp`
  (または分割した複数ファイル)に実装していく。ネイティブパッチの
  ToneLayer自体の追加・削除UIも今回は対象外(既存レイヤーの編集のみ)。
