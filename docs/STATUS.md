...
- **パッチ編集フォーム・CRUD UI** — メニュー→ファイルブラウザ→
  ...
  表示していた)。
  ネイティブパッチ(`fpe::Patch`/`ToneLayer`)の編集フォームは実装済み
  (D-036、2026-07-24)。BankDetailのネイティブパッチバンク行から
  モードレスな「ネイティブパッチ編集」ウィンドウを開き、名前/poly/
  ...
  (ネイティブパッチ/デバイスボイスパッチ両方のツリーを1つのポップアップ
  ...
  試聴できる押し続けボタンも追加(Native/Performance編集画面とは異なり、
  ...
  複数ファイル)に実装していく。ネイティブパッチのToneLayer自体の
...
- 未完了・既知の問題: SL修正自体の実機確認は、前セッションでクリック
  ...
  波形画像、ネイティブ/パフォーマンス/ドラムノートの編集フォーム、
  バンク/パッチの複製・削除UIも引き続き未着手。
- 次にやること: 次回このマシンで作業する際は、まずSL修正
  ...
  慎重な手順)。その後、他のチップ種別への対応、またはネイティブ/
  パフォーマンス/ドラムノートの編集フォーム・バンク/パッチの複製・
  削除UIに進む。
...
- やったこと: 利用者から「パッチ編集画面に登録ボタンを配置(右上/右下)。
  ...
  **実機で初めて「登録」ボタンをクリックしたところ、`unified_preset.
  ...
  他5つのデータモデル型(SwPatch/NativePatch/DrumKit/SampleZone/
...
- 未完了・既知の問題: SampleZoneのゾーン`name`フィールド欠落は今回
  ...
  種別のパラメータ範囲・接続図・波形画像、ネイティブ/パフォーマンス/
...
- 次にやること: 次回このマシンで作業する際は、SampleZoneの`name`
  ...
  実機確認する。その後、他のチップ種別への対応、またはネイティブ/
  パフォーマンス/ドラムノートの編集フォーム・バンク/パッチの複製・
  削除UIに進む。
...
- 未完了・既知の問題: SampleZoneの`name`フィールド追加、リアルタイム
  ...
  接続図・波形画像、ネイティブ/パフォーマンス/ドラムノートの編集
...
- 未完了・既知の問題: OPZ用に新規生成したWS3/5/7の波形(WS3=周波数2倍
  ...
  確認、ネイティブ/パフォーマンス/ドラムノートの編集フォーム、バンク/
  パッチの複製・削除UIは引き続き未着手・未確認。
...
- 次にやること: OPL_RHY以外はPSG系への対応、ネイティブ/パフォーマンス/
  ドラムノートの編集フォーム・バンク/パッチの複製・削除UIに進む。
...
- やったこと: 利用者から「ネイティブパッチ編集画面を実装。ToneLayer内の
  ...
  実体を引き直す」設計を踏襲した`NativePatchEditorWindow`
  (`AppContext::openNativeEditors`)を新設し、BankDetailのネイティブ
  パッチバンク行をDeviceケースと同様にクリック可能な`Selectable`に変更
  (`openNativePatchEditor()`)。エディタ本体(`renderNativePatchEditor()`)
  ...
  差分SysEx送信・試聴鍵盤は実装していない(ネイティブパッチ自体には
...
- 未完了・既知の問題: **本セッションではクリック操作の実機確認を行って
  ...
  ファイル一式のみの静的な受動確認では今回の変更点(Native
...
### 2026-07-24 (同マシン、ネイティブパッチのsw_bank/sw_prog参照もHWピッカーと同様のピッカー動作に変更)
- やったこと: 利用者から「swbank/swprogはhwパッチ編集画面と同様の
  ...
  `SwPatchPickerTarget`(`Device`/`Native`)で参照先を切り替えられる形に
  一般化した上で再利用し、`renderPatchEditor()`と全く同じ
  「パフォーマンス: {bank}/{prog} : バンク名 / パッチ名」ラベル+
  クリックでピッカーを開く方式に変更した。新規
  `openNativeSwPatchPicker()`を追加し、`renderNativePatchEditor()`から
  呼ぶようにした。一覧描画・選択・「参照解除」のロジックはDevice/Native
...
- やったこと: 利用者から「パフォーマンスパッチ編集画面を実装。波形は
  ...
  ため、今回が最初の実装になる。
  Device(D-015)/Native(D-036)編集画面と同じ「インデックスを保持し
  ...
  既存のDevice編集画面が担うため、D-036のネイティブパッチ編集画面と
  同じ判断)。
...
- 次にやること: 利用者に今回のパフォーマンスパッチ編集画面(特にLFO
  ...
  削除UIに進む。
  (→2026-07-24、下記セッションでDevice/Native編集画面のsw_bank/sw_prog
  行に「編集」ボタンを追加。)

### 2026-07-24 (同マシン、Device/Native編集画面のsw_bank/sw_prog行に「編集」ボタンを追加)
- やったこと: 利用者から「ネイティブパッチ編集、デバイスパッチ編集の
  ...
  (D-036)を、`renderPatchEditor()`(Device)・`renderNativePatchEditor()`
  (Native)双方のsw_bank/sw_prog表示行にも適用した。新規
...
- 次にやること: 利用者に今回追加した「編集」ボタン(Device/Native
  双方のsw_bank/sw_prog行)を含め、パフォーマンスパッチ編集画面全体を
  実機で目視確認してもらい、フィードバックがあれば対応する。それ以外は
  PSG系への対応、ドラムノートの編集フォーム・バンク/パッチの複製・
  削除UIに進む。
  (→2026-07-24、利用者が実機で画面遷移(Device/Native編集画面の
...
- やったこと: 利用者から「ドラムキット編集画面を実装。ドラムキット選択
  ...
  (`renderDrumNoteEditors()`/`renderDrumNoteEditor()`、Device/Native/
  ...
  「normal mode(ネイティブパッチ)/direct mode(デバイスボイスパッチ)」
  ...
  実装した(D-036/D-037のNative/Performance編集画面が「合成パラメータを
  ...
  `renderSwPatchPicker()`をそのまま再利用(D-036のNative追加と同じ判断)。
...
- やったこと: 上記セッションの実機確認で、`voice_patch_type`がADPCM系/
  ...
  「None→ネイティブ、それ以外→常に通常のHwBank/HwPatch」の二択でしか
...
- 次にやること: 利用者に今回の修正(PCM波形バンク/AWMサンプルゾーン
  ...
  ネイティブパッチが選択されてしまうという報告あり。同日中に根本原因
...
- やったこと: 直前の修正(GUI層にPCM/AWM分岐を追加)だけでは直っておらず、
  ...
  「パッチピッカーでpcmbankのprogを選択して戻るとネイティブパッチが
  ...
  「ネイティブが選択されたように見える」症状も同じ根本原因だった。
